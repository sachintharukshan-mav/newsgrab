...
export async function extractArticleImage(url: string): Promise<string | null> {
  if (!url || url === '#' || url.startsWith('http://localhost') || url.includes('news.google.com')) return null;
  ...
  try {
    ...
    const isGarbageImage = (src: string) =>
      /logo|favicon|icon|avatar|placeholder|default|J6_coFbogxh|-DR60l-|robot\.png/i.test(src);
    ...
    if (metaMatch && metaMatch[1] && !isGarbageImage(metaMatch[1])) {
      ...
      if (imgUrl.includes('googleusercontent.com') && !imgUrl.includes('J6_coFbogxh')) {
    ...
    for (const jm of jsonLdMatches) {
      try {
        ...
        if (candidate && typeof candidate === 'string' && candidate.startsWith('http') && !isGarbageImage(candidate)) {
          let cleanCandidate = candidate.replace(/&amp;/g, '&').trim();
          if (cleanCandidate.includes('googleusercontent.com') && !cleanCandidate.includes('J6_coFbogxh')) {
